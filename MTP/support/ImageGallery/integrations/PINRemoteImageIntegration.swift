...
final class PINRemoteImageIntegration: NSObject,
    ...
    fileprivate var downloadUUIDs = NSMapTable< AXPhotoProtocol,
                                                NSUUID >(keyOptions: .strongMemory,
    ...
    override init() {
        ...
        self.imageManager = PINRemoteImageManager(
            ...
            imageCache: PINRemoteImageManager.shared().cache
            // PINRemoteImageManager cache is thread safe, so we should be able to use the same cache
        )
    }
    ...
    func loadPhoto(_ photo: AXPhotoProtocol) {
        ...
        let progress: PINRemoteImageManagerProgressDownload = { [weak self] completedBytes, totalBytes in
            AXDispatchUtils.executeInBackground { [weak self] in
                ...
                self.delegate?.networkIntegration?(
                    self,
                    didUpdateLoadingProgress: CGFloat(completedBytes) / CGFloat(totalBytes),
                    for: photo
                )
            }
        }
        ...
        guard let uuid = self.imageManager.downloadImage(with: url,
                                                         options: [],
    ...
    func alternateRepresentation(with data: Data!,
                                 // swiftlint:disable:previous implicitly_unwrapped_optional
                                 options: PINRemoteImageManagerDownloadOptions = []) -> Any! {
