...
class SDWebImageIntegration: NSObject, AXNetworkIntegrationProtocol {
    ...
    fileprivate var downloadOperations = NSMapTable < AXPhotoProtocol,
                                                     SDWebImageOperation >(keyOptions: .strongMemory,
    ...
    func loadPhoto(_ photo: AXPhotoProtocol) {
        ...
        let progress: SDWebImageDownloaderProgressBlock = { [weak self] receivedSize, expectedSize, targetURL in
            AXDispatchUtils.executeInBackground { [weak self] in
                ...
                self.delegate?.networkIntegration?(
                    self,
                    didUpdateLoadingProgress: CGFloat(receivedSize) / CGFloat(expectedSize),
                    for: photo
                )
            }
        }
        ...
        guard let operation = SDWebImageManager.shared.loadImage(with: url,
                                                                 options: [],
                                                                 progress: progress,
                                                                 completed: completion) else { return }
